...
CMD:goto(playerid, params[]) //FLAGA ADMINA
{
	...
	SendClientMessage(playerid, -1, "Teleportowa³eœ siê do gracza");
	return 1;
}
...
CMD:tp(playerid, params[])
{
	...
	if(!strcmp(command, "salon", true)) {
		SendClientMessage(playerid, COLOR_LIGHTBLUE, "Pomyœlnie siê teleportowa³eœ.");
...
CMD:slap(playerid, params[])
{
	...
	if(!IsPlayerAdminLevel(playerid, 1)) return NoAccessMessage(playerid);
	if (sscanf(params, "us[64]", targetid, reason)) return SendClientMessage(playerid, COLOR_LIGHTRED, "U¯YCIE: /slap [id/nick] [powód]");
	...
	format(string, sizeof(string), " %s da³ slapa graczowi %s, powód: %s", pName(playerid), pName(targetid), reason);
...
CMD:spec(playerid, params[])
{
	if(IsPlayerAdminLevel(playerid, 1000) || IsPlayerAdminLevel(playerid, 6) || IsPlayerAdminLevel(playerid, 1)) {
		...
		if(GetPVarInt(playerid, "Spectating") == 0) {
			...
			//format(string, sizeof(string), "Aby wy³¹czyæ podgl¹d u¿yj komendy /spec %d", targetid);
			...
			SendClientMessage(playerid, COLOR_GREY, "Wy³¹czy³eœ podgl¹d SPEC");
